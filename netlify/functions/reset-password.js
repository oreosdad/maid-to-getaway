...
exports.handler = async (event) => {
  ...
  if (action === 'update_email') {
    ...
    // Try to update auth — if 404, user has no auth account, just update profile
    const { ok, status } = await authAdminRequest('PUT', `/users/${userId}`, { email, email_confirm: true });
    if (!ok && status !== 404) {
      // Real error — still try to update profile
      console.error('Auth email update failed, updating profile only');
    }
  ...
  if (action === 'delete_user') {
    ...
    // Delete from auth (ignore 404 — profile-only users)
    const { ok, status } = await authAdminRequest('DELETE', `/users/${userId}`);
    if (!ok && status !== 404) {
      return { statusCode: 400, body: JSON.stringify({ error: 'Could not delete from auth' }) };
    }

    // Delete job_assignments (service role bypasses RLS)
    ...
    if (profErr) {
      ...
      return { statusCode: 400, body: JSON.stringify({ error: 'Profile delete failed: ' + profErr.message }) };
    }
