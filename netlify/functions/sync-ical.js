  ...
  for (const prop of properties) {
    ...
    for (const feed of feeds) {
      ...
      for (const evt of validEvents) {
        ...
        // Also find earliest sibling checkin to use as our effective checkin
        let effectiveCheckinDT = checkinDT;

        if (groupMap[prop.id]) {
          const siblings = groupMap[prop.id];
          // Get next day string for lookahead
          const nextDay = new Date(cleanDate + 'T12:00:00');
          nextDay.setDate(nextDay.getDate() + 1);
          const nextDayStr = nextDay.toISOString().split('T')[0];

          for (const sibId of siblings) {
            // Check same day AND next day sibling jobs
            for (const lookupDate of [cleanDate, nextDayStr]) {
              const sibKey = `${sibId}|${lookupDate}`;
              const sibJob = jobsByPropDate[sibKey];
              if (!sibJob) continue;

              // If sibling has a checkin, check if it creates a tight window with our checkout
              if (sibJob.checkinDT && checkoutDT) {
                const gapHrs = (new Date(sibJob.checkinDT) - new Date(checkoutDT)) / 36e5;
                if (gapHrs >= 0 && gapHrs < 8) {
                  isShortWindow = true;
                  if (!effectiveCheckinDT || new Date(sibJob.checkinDT) < new Date(effectiveCheckinDT)) {
                    effectiveCheckinDT = sibJob.checkinDT;
                  }
                }
              }

              // If sibling has a checkout on same day, check against our checkin
              if (sibJob.checkoutDT && checkinDT) {
                const gapHrs = (new Date(checkinDT) - new Date(sibJob.checkoutDT)) / 36e5;
                if (gapHrs >= 0 && gapHrs < 8) isShortWindow = true;
              }
            }
          }
          // Recalculate isShortWindow with effective checkin
          if (!isShortWindow && checkoutDT && effectiveCheckinDT) {
            const gapHrs = (new Date(effectiveCheckinDT) - new Date(checkoutDT)) / 36e5;
            if (gapHrs >= 0 && gapHrs < 8) isShortWindow = true;
          }
        }

        // Use effective checkin (earliest sibling or own next booking)
        const finalCheckinDT = effectiveCheckinDT;

        // Store this job's times for sibling lookups
        jobsByPropDate[`${prop.id}|${cleanDate}`] = { checkoutDT, checkinDT: finalCheckinDT };
        ...
        const { error: insertErr } = await db.from('jobs').insert({
          ...
          guest_checkin:   finalCheckinDT || checkinDT,
          status:          'unassigned',
          source:          feed.source,
          ical_uid:        evt.uid,
