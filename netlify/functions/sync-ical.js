    ...
    for (const evt of validEvents) {
      ...
      if (groupMap[prop.id]) {
        ...
        const { data: siblingJobs } = await db
          .from('jobs')
          .select('property_id, scheduled_date, guest_checkout, guest_checkin')
          ...
          for (let d = 0; d <= 30; d++) {
            ...
            if (inMemory && !allSibJobs.find(j => j.property_id === sibId && j.scheduled_date === ldStr)) {
              allSibJobs.push({
                ...
                guest_checkin: inMemory.checkinDT
              });
            }
          }
        }

        console.log(`[DUPLEX DEBUG] ${prop.name} on ${cleanDate}: checkoutDT=${checkoutDT}, checkinDT=${checkinDT}`);
        console.log(`[DUPLEX DEBUG] Found ${allSibJobs.length} sibling jobs:`, JSON.stringify(allSibJobs.map(j => ({name: j.property_id, date: j.scheduled_date, checkin: j.guest_checkin}))));

        for (const sibJob of allSibJobs) {
          const sibCheckin = sibJob.guest_checkin;
          const sibCheckout = sibJob.guest_checkout;

          if (sibCheckin && checkoutDT) {
            const gap = (new Date(sibCheckin) - new Date(checkoutDT)) / 36e5;
            if (gap >= 0) {
              if (gap < 8) isShortWindow = true;
              if (!effectiveCheckinDT || new Date(sibCheckin) < new Date(effectiveCheckinDT)) {
                effectiveCheckinDT = sibCheckin;
              }
            }
          }
