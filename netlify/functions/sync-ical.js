  ...
  for (const prop of properties) {
    ...
    for (const feed of feeds) {
      ...
      for (const evt of events) {
        ...
        // Only sync future or today's events
        if (evt.dtstart < today) continue;

        // Check-in day = DTEND of iCal (Airbnb/VRBO convention: DTEND = checkout = next check-in)
        // Cleaning date = DTEND (the day guests check out, cleaner comes in)
