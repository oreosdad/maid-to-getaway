...
exports.handler = async (event) => {
  ...
  const { data: groupMembers } = await db.from('property_group_members').select('property_id, group_id');
  const groupMap = {};
  if (groupMembers) {
    const byGroup = {};
  ...
  for (const prop of properties) {
    ...
    for (const feed of feeds) {
      ...
      for (const evt of validEvents) {
        ...
        // Keep the earliest checkin if multiple feeds for same property
        if (!jobsByPropDate[key] || (checkinDT && new Date(checkinDT) < new Date(jobsByPropDate[key].checkinDT || '9999'))) {
          jobsByPropDate[key] = { checkoutDT, checkinDT, propId: prop.id };
        }
      }
    }
  }
  ...
  for (const { prop, feed, validEvents } of allFeedData) {
    for (const evt of validEvents) {
      ...
      if (groupMap[prop.id]) {
        ...
        for (const sibId of groupMap[prop.id]) {
          // Check same day, next day, and up to 7 days ahead for sibling checkins
          for (let dayOffset = 0; dayOffset <= 7; dayOffset++) {
            const lookupDay = new Date(cleanDate + 'T12:00:00');
            lookupDay.setDate(lookupDay.getDate() + dayOffset);
            const lookupDate = lookupDay.toISOString().split('T')[0];
            const sibJob = jobsByPropDate[`${sibId}|${lookupDate}`];
            if (!sibJob) continue;

            if (sibJob.checkinDT && checkoutDT) {
              const gap = (new Date(sibJob.checkinDT) - new Date(checkoutDT)) / 36e5;
              if (gap >= 0) {
                // Flag as short window if under 8 hours
                if (gap < 8) isShortWindow = true;
                // Always use earliest sibling checkin as effective checkin for display
            ...
            if (sibJob.checkoutDT && checkinDT) {
              const gap = (new Date(checkinDT) - new Date(sibJob.checkoutDT)) / 36e5;
              if (gap >= 0 && gap < 8) isShortWindow = true;
            }
          }
        }
